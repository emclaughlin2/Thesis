...
For both the \pythia and \herwig generators, each sample is generated with a minimum hard-scattering \pT{} threshold to ensure sufficient statistics in the high-\pT{} jet region.
...
Finally, the fully efficient truth-level leading jet \pT{} ranges for each sample are determined by matching the resulting spectra to those from minimum-bias and/or lower-threshold filtered samples, ensuring a continuous spectrum across overlapping regions of phase space. 
...
\end{tabular}
\caption{Summary of MC samples used in the p+p underlying event measurement. For each sample, the leading jet transverse momentum range, generator type, minimum $p_{T}$ for generated hard scattering, sample cross section with filter efficiency applied and number of events information are included.}
