...
Each EMCal sector has six 64 channel interface boards used to communicate with the detector for setting and monitoring the detector state.
...
Here each IHCal and OHCal half sector has one 24 channel interface board and these are also used to set and monitor the detector state including SiPM temperatures, bias voltages and offsets, leakage current and pre-amplifier gain mode by connecting to a slow control communication board.
...
This nominal reconstruction procedures is outlined below in Section \ref{sec:nominal_calorimeter_reconstruction}.
This dataset was also used to investigate a set of characteristic failure modes in the calorimeter digitization and read out process.
This investigation process is also described in detail below in Sections \ref{sec:diagnosing_adc_bit_issues}, \ref{sec:online_adc_firmware_fixes}, and \ref{sec:offline_reconstruction_software_fixes}.
...
The Analog Device AD9257 on the boards perform a digitization of each analog signal to 14-bit digital precision operating at 6x the Beam Crossing Clock (106 ns / 6 = 17.6 ns).
...
\subsection{Nominal Calorimeter Tower Reconstruction}
\label{sec:nominal_calorimeter_reconstruction}
...
Figure~\ref{fig:waveform_template} shows the shape of a good digitized-signal waveform with the waveform template fit overlaid.

\begin{figure}
    \centering
    \includegraphics[width=0.7\linewidth]{figures/chapter3/waveform_template.png}
    \caption{Calorimeter digitized signal waveform given in ADC counts digitized for 31 time samples. The waveform template function is fit to this waveform and overlaid to show the extracted waveform shape.}
    \label{fig:waveform_template}
\end{figure}
...
Figure~\ref{figure:bad_tower_masks} shows examples of hot tower masks for EMCal, IHCal and OHCal for a single run used in the Run 2024 \auau{} analysis described in detail in Chapter \ref{ch:detdeta_auau}.
There are three ways for a tower to be considered bad on a run by run basis: a tower can be dead, cold or hot.
To determine a tower's status on a run by run basis, the hits above a threshold are collected for each tower and the tower-by-tower distribution of these hits is studied.
In towers where the number of hits above a threshold is exactly zero, the tower is considered dead for the run. 
There are ~300 dead towers in the EMCal due to unrecoverable interface board hardware issues and no dead towers in either of the HCals.
A tower is considered cold if the number of hits above a threshold is either 10\% or 5$\sigma$ below the median number of hits above a threshold for all towers in that calorimeter layer. 
A small number, $< 5$, of towers are considered cold in either the EMCal or HCal. 
A tower is considered hot if the number of hits above a threshold is 5$\sigma$ above the median number of hits above a threshold for all towers in that calorimeter layer or more 1\% of the tower's waveforms above some threshold for a run have a bad quality parameter from the template fit. 
A small number of towers are considered hot in the HCal ($< 5$) and a moderate number of towers are considered hot in the EMCal (~50).

Finally, there are small contributions to the bad tower masking on the event-by-event level from towers with a waveform fit quality parameter outside of the nominal range. 
Less than 1\% of events have a tower in any of the calorimeter layers with a bad quality parameter that is not already included in the run-by-run bad tower mask. 
A systematic study masking these abnormal waveforms on the event-by-event level found the effect on the measurements in Chapters \ref{ch:detdeta_auau} and \ref{ch:ue_in_pp} to be negligible. 

\begin{figure}
    \centering
    \includegraphics[width=0.48\textwidth]{detdeta/Figures/hot_tower_masks/emcal_hot_tower_mask_54912.png} 
    \includegraphics[width=0.48\textwidth]{detdeta/Figures/hot_tower_masks/ihcal_hot_tower_mask_54912.png} 
    \includegraphics[width=0.48\textwidth]{detdeta/Figures/hot_tower_masks/ohcal_hot_tower_mask_54912.png} 
    \caption{Example of hot tower masks for EMCal (top left), IHCal (top right) and OHCal (bottom) for a single run used in the Run 2024 \auau{} analyses. Each plot shows a rolled-out depiction of the calorimeter acceptance with tower index values in pseudorapidity ($i\eta$) shown horizontally and tower index values in azimuth ($i\phi$) shown vertically. Towers in green are masked.}
    \label{figure:bad_tower_masks}
\end{figure}

\subsection{Diagnosing ADC Bit Issues}
\label{sec:diagnosing_adc_bit_issues}

An investigation into the causes for the bad towers found in the calorimeter reconstruction chain was performed using the Run 2023 commissioning dataset.
This investigated focused on using the quality parameter from the calorimeter waveform template fit to identify abnormal waveforms. 
Two main cases of abnormal behavior for calorimeter waveforms were found in the Run 2023 commissioning dataset.
The first case of abnormal waveform shape was characterized by waveforms with gaps in ADC counts of $2^{n}$ and this behavior was found in ~10-20 channels per run and persists in every event for an effected channel across an entire run. 
The second case was characterized by waveforms with stuck bit or "bit flip" for a time sample in the digitized waveform.
This behavior was found in ~10 channels per run and was found to occur intermittently throughout a run.

\begin{figure}
    \centering
    \includegraphics[width=\linewidth]{figures/chapter3/bit_flip_shift_examples.png}
    \caption{Examples of abnormal waveforms with bit flip (top) or $2^{n}$  gaps of ADC counts (middle and bottom) behavior. Left plots show the abnormal waveforms for a single event. Right plot show a persistency plot of the channels waveforms overlaid for an entire run.}
    \label{fig:bit_flip_shift_examples}
\end{figure}

\subsection{Online ADC Firmware Fixes}
\label{sec:online_adc_firmware_fixes}

For the first case of abnormal waveforms where the waveforms always exhibited a gap of $2^{n}$ between digitized waveform values for particular time samples the binary sequence of the waveform values was studied in detail.
...
\label{sec:offline_reconstruction_software_fixes}
...
In the default calorimeter reconstruction chain, waveforms from channels with stuck bit issues were masked out, since energy contributions from towers with high value stuck bits can easily pollute a physics measurement, especially studies of rare high-energy jet and photon measurements.
...
In this default reconstruction chain, waveforms with stuck bit issues have a characteristic value for the quality parameter (denoted as $\chi^2/NDF$) which is different from the nominal value range.
These abnormal waveforms have a $\chi^2/NDF$ value of greater than 10 times the nominal value range.

\begin{figure}
    \centering
    \includegraphics[width=0.6\linewidth]{figures/chapter3/emcal_chi2.png}
    \caption{EMCal channel $\chi^2/NDF$ factor extracted from the waveform template fit versus extracted energy E. Three trend lines included in the plot, a vertical band in the low EMCal tower energy range with varied $\chi^2/NDF$ from noise, a main band of good signals featuring a dilute correlation between EMCal tower energy and $\chi^2/NDF$ and a horizontal band at very high $\chi^2/NDF$ values where read out failures (stuck bit and bit misalignment issues) are located.}
    \label{fig:emcal_chi2}
\end{figure}
...
This well-defined goal was to recover as many single sample bit flip waveforms as possible from waveforms with characteristic high value (bits $2^{11}$ to $2^{13}$) stuck bit behavior which have been previously successfully identified using high $\chi^2/NDF$ signature from the template fit.
An additional goal of this recovery method was to only recover stuck bit waveforms and avoid performing the recovery procedure on other types of abnormal waveforms (ie. waveforms with a shifted bit-stream, waveforms with abnormal shape, and waveforms with lower value stuck bits (bits $2^{10}$ and below)).
...
The recovery method is defined below and described using pseudocode in Figure \ref{figure:bit_flip_recovery_algorithm}.
First, possible stuck bit waveform candidates are identified using the fit quality ($\chi^2/NDF$) and pedestal values returned from the waveform's fit to the default calorimeter template.
...
\section{sPHENIX Calorimeter Calibration}
...
This simulation-based target mass is obtained from a full detector simulation of \auau{} collisions generated with \texttt{HIJING}~\cite{hijing} and \pp{} collisions generated with \texttt{PYTHIA8}~\cite{pythia8,pythia8_detroittune} passed through a \textsc{geant-4}~\cite{GEANT4} simulation of the sPHENIX detector and its digitization chain.
The calibration procedure contains two steps: first a relative calibration is applied to enforce $\phi$ symmetry and eliminate tower-by-tower gain differences.
In this tower slope method, each tower spectrum is fit to the reference spectrum using the fit function $A\times f(gE)$, where $A$ is a scale factor and $g$ is the gain parameter, in the tower energy region between 0.2 and 0.8 GeV.
...
The tower slope mean and standard deviation values are then recalculated after removing outliers and tower-by-tower gain values are extracted again from the fit to the reference spectra.
...
This procedure is iterated through many times (20-30) until only sub-statistical fluctuations in the observed $\pi^{0}$ mass peak position are observed.

For the EMCal absolute energy scale calibration for the Run 2024 \auau{} dataset, the 20M minimum bias events from same dataset used for the \detdeta analysis were used for data and 10M minimum bias \texttt{HIJING} events were used for the simulation-based target mass values.
Here the data to simulation comparisons of the $\pi^{0}$ distribution showed that the $\pi^{0}$ mass width was larger in data than in the \texttt{HIJING} events.
The data to simulation disagreement in the $\pi^{0}$ mass width can be seen in Figure~\ref{fig:emcal_pi0} (left).
Additionally, $\pi^{0}$ mass peak position used for the EMCal calibration is shifted in both data and simulation from the Particle Data Group value~\cite{ParticleDataGroup:2024cfk} because of effects from the convolution of the falling energy spectrum with the resolution in the energy response of the EMCal clusters used for the calibration.
...
Figure~\ref{fig:emcal_pi0} compares the EMCal di-cluster distributions before and after simulation EMCal cluster energy smearing and shows the good agreement in the EMCal di-cluster distribution between data and simulation with this smearing applied.

Figure~\ref{fig:emcal_pi0_eta_dependence} shows the $\pi^{0}$ mass peak position and width as a function of cluster $\eta$ with and without the simulation EMCal cluster energy smearing.
In Figure~\ref{fig:emcal_pi0_eta_dependence}, the x-axis $i\eta$ value represents the EMCal cluster $\eta$ from the di-cluster pair mapped onto the EMCal tower $i\eta$ indices which range from 0 to 95.
This mapping is done to allow for binning of the EMCal cluster pairs dominated by $\pi^{0}$ candidates into discrete tower slices for the calibration procedure.
Additionally, a small dip in the $\pi^{0}$ mass peak position in the central $\eta$ region of the EMCal is also observed in both data and simulation.
This noticeable change in the energy response for towers at central $\eta$ is due to the projective nature of the EMCal towers where central $\eta$ towers have a smaller surface area compared to towers at larger $\eta$ values. 

\begin{figure}
    \centering
    \includegraphics[width=\textwidth]{figures/chapter3/emcal_pi0_mass_width_versus_eta.png} 
    \caption{EMCal $\pi^{0}$ mass peak position (left) and relative width (right) for simulation (red), simulation with cluster energy smearing (green), data calibrated to the un-smeared simulation (blue) and data calibrated to the smeared simulation (black). $\eta$ averaged quantities are shown as dotted (data) and dashed (simulation) lines for easy comparison.}
    \label{fig:emcal_pi0_eta_dependence}
\end{figure} 

\begin{figure}
    \centering
    \includegraphics[width=0.48\textwidth]{figures/chapter3/emcal_calib_pi0_dist_before_mc_smear.png} 
    \includegraphics[width=0.48\textwidth]{figures/chapter3/emcal_pi0_final_prc_edit_label.pdf} 
    \caption{EMCal di-cluster invariant mass distribution from Run 2024 \auau{} data (black points) and \geant simulation of \texttt{HIJING} events (blue histogram) before (left) and after (right) simulation EMCal cluster energy smearing.}
    \label{fig:emcal_pi0}
\end{figure}
...
Figure~\ref{fig:emcal_noise_effects} shows a 3.5\% effect on the EMCal tower energy resolution from the inclusion of realistic EMCal tower noise for towers $\sim 1 \mathrm{GeV}$ which dominate the calibration cluster candidates. 
...
Similar to Figure~\ref{fig:emcal_pi0_eta_dependence}, in Figures~\ref{fig:emcal_pi0_mass_vars} and \ref{fig:emcal_pi0_width_vars}, the x-axis $i\eta$ value represents the EMCal cluster $\eta$ from the di-cluster pair mapped onto the EMCal tower $i\eta$ indices which range from 0 to 95.
...
\begin{figure}
    \centering
    \includegraphics[width=\textwidth]{figures/chapter3/emcal_calib_mc_data_var_pi0_mass.png} 
...
\begin{figure}
    \centering
    \includegraphics[width=\textwidth]{figures/chapter3/emcal_calib_mc_data_var_pi0_width.png} 
...
The IHCal and OHCal are both calibrated to the electromagnetic energy scale using the minimum ionizing particle (MIP) energy depositions from cosmic ray muons from data taken during no-beam periods. 
Cosmic ray data collection for HCal calibration were taken periodically throughout the year-long data-taking period.
Time-dependent calibrations were developed which use collections of cosmic ray data taken closest in time to each beam data-taking period to include small time-dependent detector condition effects on gain variations in the calibration. 
...
Since this selection requires events where the muon traverses a majority of the tower's scintillating tiles, it produces a narrow, characteristic MIP energy peak needed for a precise calibration.
...
Finally, a sampling fraction factor is applied to calibrate the measured scintillator MIP energy distribution to an estimate of the energy in the full HCal tower (scintillator and absorber material).
...
The effect of tower reconstruction via fitting to the calorimeter waveform template versus zero suppression (ZS) algorithm as well as the application of an additional cross calibration factors for zero suppressed towers was extensively studied since the low energy towers are a significant contributor to soft physics analyses like the $\detdeta$ and underlying event measurements.
An additional calibration was applied to the ZS tower energies to account for the ZS algorithm, based on waveform sample 6 - sample 0, underestimating the tower energy. 
...
The first method is by looking at the correlation of $ADC_{\text{template fit}}$ to $ADC_{\text{sample 6 – sample 0}}$ to look at the energy dependence on the relationship between the template fit and ZS energy values. 
The second method is by looking at the mean ratio of $ADC_{\text{template fit}}$/$ADC_{\text{sample 6 – sample 0}}$ for each tower entry in a set energy range to look for tower-by-tower effects on the relationship between the template fit and ZS energy values.

\begin{figure}
    \centering
    \includegraphics[width=0.8\linewidth]{detdeta/Figures/ZS_template_fit/emcal_zs_vs_template_fit.png}
    \caption{Comparison of EMCal waveform processing with zero suppression versus template fit. The correlation between a tower's ADC value from ZS versus from template fit is shown in left plots for two different ADC ranges [-100,1000] ADC (top left) and [-100,100] ADC (bottom left). The ratio of ADC value from ZS versus ADC value from template fit on a tower-by-tower basis is shown in right plots for two different ranges [100,1000] ADC (top right) and [-100,100] ADC (bottom right).}
...
In the EMCal $ADC_{\text{template fit}}$ versus $ADC_{\text{sample 6 – sample 0}}$ correlation plot in Figure \ref{fig:emcal_zs_template_corr} (upper left), there is a main trend line at 1, a broader trend line below 1, and additional structure in region below 100 ADC. 
...
However, the band below 1 for the OHCal is broader than in the EMCal case. Additionally, in the OHCal case, there are still the correlated and anti-correlated trend lines for the processing of low energy waveforms using the template fit versus the ZS method. 
...
Further investigation into the correlated and anti-correlated trend lines seen in the low ADC region was conducted using the OHCal and EMCal waveforms. 
...
The fully decreasing waveform in Figure \ref{fig:noisy_waveforms} (left) when processed by template fit returns peak-pedestal value of 18.9 ADC and when processed with the ZS algorithm returns a peak-pedestal value of -10 ADC.
The fully increasing waveform in Figure \ref{fig:noisy_waveforms} (right) when processed by template fit returns a peak-pedestal value of -20.5 ADC and when processed with the ZS algorithm returns a peak-pedestal value of 11 ADC.
...
\begin{figure}
    ...
    \caption{Examples of noisy waveforms from commissioning beam data with 31 time samples read out.}
    \label{fig:noisy_waveforms}
\end{figure}

Understanding how the template fit processes these waveforms with long range noise helps to explain the structure and scale of Figures \ref{fig:emcal_zs_template_corr} (bottom right) and \ref{fig:ohcal_zs_template_corr} (bottom right). 
These plots show for each tower, denoted by its $i\eta,i\phi$ location, the ratio of the mean ADC value extracted from the template fit to the mean ADC value extracted from the ZS algorithm, $\overline{ADC}_{\text{template fit}}$/$\overline{ADC}_{\text{sample 6 – sample 0}}$, in the region [-50,50] ADC. 

For the EMCal, this ratio $\overline{ADC}_{\text{template fit}}$/$\overline{ADC}_{\text{sample 6 – sample 0}}$ in low energy range (+/- 50 ADC = +/-75 MeV) shows a consistent value across EMCal towers of 0.57. 
This ratio should be related to the two competing effects at low energy: low energy signals which produce correlated values from the two waveform reconstruction methods (template fit and ZS) and long range noise waveforms with produce anti-correlated values from the two waveform reconstruction methods.
The OHCal ratio $\overline{ADC}_{\text{template fit}}$/$\overline{ADC}_{\text{sample 6 – sample 0}}$ in the low energy range (+/- 50 ADC = +/-150 MeV) is around 0.74 but has two blue areas that correspond to four ADC boards with high pedestal RMS (~10 ADC), that seem to be main contributors to the anti-correlation band seen in the full detector correlation plot. 
Once again the two trends, correlated behavior between the template fit and ZS results from real low energy signals and anti-correlated behavior from long range noise waveforms, compete with one another and the noisier boards have a lower mean ratio due to a larger contribution of long range noise waveforms with anti-correlated behavior between the two waveform processing methods.
The IHCal ratio $\overline{ADC}_{\text{template fit}}$/$\overline{ADC}_{\text{sample 6 – sample 0}}$ in the low energy range (+/- 50 ADC = +/-25 MeV) also has two blue shaded areas correspond to 2 ADC boards with high pedestal RMS (~6 ADC) which exhibit the same behavior as the OHCal noisy boards.

The absolute calibration established for all three calorimeters uses the template fit where the peak time is a free parameter. 
...
To determine this cross calibration, the $\overline{ADC}_{\text{template fit}}$/$\overline{ADC}_{\text{sample 6 – sample 0}}$ ratio for well defined waveforms well above the noise contributions show in Figures \ref{fig:emcal_zs_template_corr}, \ref{fig:ohcal_zs_template_corr}, \ref{fig:ihcal_zs_template_corr} (top right) is used. 
The waveforms that determine these ratios can be mapped to the correlation plots in Figures \ref{fig:emcal_zs_template_corr}, \ref{fig:ohcal_zs_template_corr}, \ref{fig:ihcal_zs_template_corr} (top left), where they make up both the band structure at 1 and lower band structure below 1. 
For the EMCal, most towers have a value for the $\overline{ADC}_{\text{template fit}}$/$\overline{ADC}_{\text{sample 6 – sample 0}}$ ratio of nearly 1 with some parts of the detector having a value around 0.8, due to these parts of the detector not being timed in exactly at time sample 6. 
These towers with ratio value around 0.8 contribute to the band below 1 in the correlation plot. 
...
Additionally, in the OHCal and IHCal, the tower-by-tower values for the $\overline{ADC}_{\text{template fit}}$/$\overline{ADC}_{\text{sample 6 – sample 0}}$ ratio have a small $|\eta|$ dependence due to cable length differences resulting in overall timing differences across the detectors.

These tower-by-tower ratios are used as a cross calibration for the energy in GeV from the ZS waveform processing method. 
Applying the cross calibration, the energy of a waveform processed with the ZS method is:
\begin{equation}
...
Through in-depth studies of the ZS and template fit waveform processing methods, it is now understood that the low energy region of the calorimeter tower energy spectra is quite noisy and that this noise is processed differently using the template fit versus ZS method. 
...
Finally, the cross calibration factors are applied to the ZS processed waveforms to establish the correct energy scale for these towers and ensure a continuous energy spectrum across the boundary between the ZS and template fit processing methods. 
...
A range of thresholds from 10 to 50 ADC is tested to find at what point the minimum bias distribution of calorimeter total $\ET$ value, for example, becomes insensitive to the ZS + template fit processing method, indicating that there are no longer noise contributions included in the offset or pedestal of the total calorimeter energy. 
...
In raising the ZS threshold, more of these increasing noise waveforms are then processed with the ZS method, and the total $\ET$ rises and finally stabilizes around a threshold of 30 ADC. 
...
\begin{figure}
    ...
    \caption{EMCal (top left), IHCal (top right) and OHCal (bottom) total $\ET$ distributions for minimum bias Au+Au data with ZS cross calibration factors applied for 5 different ZS thresholds.}
    \label{fig:ET_w_zs}
\end{figure}
...
\begin{figure}
    ...
    \caption{Comparison of reconstructed EMCal $\detdeta$ measurements (top) and ratios to full template fit EMCal $\detdeta$ (bottom) from minimum bias Au+Au data processed using ZS + template fit method for thresholds between 10 and 50 ADC with (left) and without (right) tower-by-tower ZS cross calibration factors applied.}
...
This occurs because real signal energies are underestimated when using the ZS method of waveform processing if the signal peak is not at exactly time sample 6.
...
\begin{figure}
    ...
    \caption{Comparison of reconstructed IHCal $\detdeta$ measurements (top) and ratios to full template fit IHCal $\detdeta$ (bottom) from minimum bias Au+Au data processed using ZS + template fit method for thresholds between 10 and 50 ADC with (left) and without (right) tower-by-tower ZS cross calibration factors applied.}
    \label{fig:ihcal_zs_detdeta}
\end{figure}

In Figure \ref{fig:ohcal_zs_detdeta}, the reconstructed-level OHCal $\detdeta$ sees similar trends to the IHCal: that applying any ZS threshold has a small effect on the reconstructed $\detdeta$ and that in order to not underestimate the energy of waveforms processed by the ZS method and have calorimeter energies that are robust to ZS threshold variations, a tower-by-tower ZS cross calibration factor must be applied.
...
\begin{figure}
    ...
    \caption{Comparison of reconstructed OHCal $\detdeta$ measurements (top) and ratios to full template fit OHCal $\detdeta$ (bottom) from minimum bias Au+Au data processed using ZS + template fit method for thresholds between 10 and 50 ADC with (left) and without (right) tower-by-tower ZS cross calibration factors applied.}
    \label{fig:ohcal_zs_detdeta}
\end{figure}

\subsubsection{ZS + template fit method on pedestal data}

To further ensure the ZS + template fit method does not bias the calorimeter energy spectra from pedestal noise contributions, the ZS threshold variations were also studied on pedestal data.
...
\begin{figure}
    ...
    \caption{Left: Comparison of reconstructed-level $\detdeta$ from EMCal (top), IHCal (middle) and OHCal (bottom) pedestal data processed using template fit only and ZS + template fit method. Middle \& Right: Comparison of reconstructed-level $\detdeta$ from pedestal data processed using ZS + template fit method for different ZS thresholds. Plots in right column are included to more clearly show the results included in the center column plots with values close to zero.}
