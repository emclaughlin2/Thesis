...
\begin{figure}
    ...
    \caption{Example of hot tower masks for EMCal (top left), IHCal (top right) and OHCal (bottom) for a single run used in the Run 2024 \auau{} analyses.}
...
This behavior was found in ~10 channels per run and was found to occur intermittantly throughout a run.
...
The constrained goal was to recover as many single sample bit flip waveforms as possible from waveforms with characteristic high value (bits $2^{11}$ to $2^{13}$) stuck bit behavior which have been previously successfully identified using high $\chi^2/NDF$ signature from the template fit.
...
This simulation-based target mass is obtrained from a full detector simulation of \auau{} collisions generated with \texttt{HIJING}~\cite{hijing} and \pp{} collisions generated with \texttt{PYTHIA8}~\cite{pythia8,pythia8_detroittune} passed through a \textsc{geant-4}~\cite{GEANT4} simulation of the sPHENIX detector and its digitization chain.
...
For the EMCal absolute energy scale calibration for the Run 2024 \auau{} dataset, data to simulation comparisons of the $\pi^{0}$ distribution showed that the $\pi^{0}$ mass width was larger in data than in simulation when using nominal \texttt{HIJING} events and the default sPHENIX \textsc{geant-4} simulation and digitization.
\textcolor{red}{Would be good to include information on the simulation and data datasets used for the EMCal calibration here.}
...
This noticable change in the energy response for towers at central $\eta$ is due to the projective nature of the EMCal towers where central $\eta$ towers have a smaller surface area compared to towers at larger $\eta$ values. 
...
\begin{figure}
    ...
    \caption{EMCal di-cluster invariant mass distribution from Run 2024 \auau{} data (black points) and geant-4 simulation of \texttt{HIJING} events (blue histogram) before (left) and after (right) simulation EMCal cluster energy smearing.}
    \label{fig:emcal_pi0}
\end{figure}
...
In the EMCal $ADC_{\text{template fit}}$ versus $ADC_{\text{sample 6 – sample 0}}$ correlation plot in Figure \ref{fig:emcal_zs_template_corr} (upper left), there is a main trendline at 1, a broader trendline below 1, and additional structure in region below 100 ADC. 
...
Through in-depth studies of the ZS and template fit waveform processing methods, it is now understood that the low energy region of our tower energy spectra is quite noisy and that this noise is processed differently using the template fit versus ZS method. 
