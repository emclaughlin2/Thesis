...
The calorimeter topoclustering algorithm groups individual calorimeter towers into contiguous clusters designed to reconstruct energy deposits from showering particles.
Topoclusters are a particular clustering method used to cluster calorimeter signals while suppressing electronic noise and pile-up contributions. 
To limit noise contributions, the clustering algorithm evaluates a signal-to-noise significance for each tower, defined as the tower's energy divided by its expected noise~\cite{ATLAS:2017_topological_cluster_performance,ATLAS:2024_topological_cluster_timing}. 
Topoclusters are then grown around "seed" towers with high significance in the following way:
\begin{itemize}
    \item Seed threshold ($S$): Towers with $|E_{\mathrm{tower}}| > S\,\sigma_{\mathrm{noise}}$ initiate clusters, typically with $S=4$.
    \item Neighbor growth threshold ($N$): Adjacent towers with $|E_{\mathrm{tower}}| > N\,\sigma_{\mathrm{noise}}$ are iteratively added to expand the cluster, typically with $N=2$.
    \item Peripheral threshold ($P$): Boundary towers are included bordering the neighboring towers with an even lower threshold, often $P=0$, to capture the full shower energy.
\end{itemize}
where each threshold is defined in units of the tower noise: $\sigma_{\mathrm{noise}}$.

The clusters are grown iteratively outwards from the cluster seeds using the neighboring towers. Here neighboring towers that satisfy the $N$ threshold can themselves contribute additional neighbors and expanded the cluster until no further cells meet the criteria. Towers are added to clusters if they are adjacent in either $\eta$ or $\phi$ within the same calorimeter layer or if they overlap in the $(\eta,\phi)$ plane across calorimeter layers (EMCal, IHCal, and OHCal in sPHENIX). 
The inclusion of boundary cells at the $P$ threshold helps to preserve the shower shape with still minimal noise contamination. An example of topocluster growth in sPHENIX is shown in the distribution of topoclusters across calorimeter layers in a simulated dijet event display (Figure~\ref{fig:topocluster_event_display}). 
Importantly, this cluster formation ordering of seed, neighboring and peripheral towers makes topoclusters far less sensitive to noise than traditional distance-only based clustering, since isolated noise fluctuations typically fail the seed threshold, and lower significance noise towers are excluded unless supported by adjacent high-significance signals. 

\begin{figure}
    \centering
    \includegraphics[width=\linewidth]{figures/chapter5/topocluster_event_display.png}
    \caption{Display of topocluster boundaries across the sPHENIX EMCal (left), IHCal (middle) and OHCal (right) calorimeter layers for a \pythia p+p dijet event. The transverse region is shown in gray shading.}
...
While the resolutions are very similar for the EM shower case, in the hadronic shower case, the resolutions obtained with the $\langle3\sigma_{ped}\rangle$ and $\langle4\sigma_{ped}\rangle$ thresholds are significantly better and are found to be very similar to each other. 
...
\begin{figure}[h!]
  ...
  \end{subfigure}
  \caption{$E/p$ distributions for EM and hadronic particles using the optimal topocluster noise thresholds for Run 2024 p+p noise conditions.}
...
\begin{figure}
    \centering
    \includegraphics[width=0.7\linewidth]{ueinpp/Figures/topocluster_noise/transverse_ET.png}
