...
\begin{figure}
    \centering
    \includegraphics[width=0.7\linewidth]{figures/chapter5/h_ue_pt_transverse_Topoclusters.png}
    \caption{Comparison of reconstruction-level mean transverse region energy distributions from topoclusters (solid points) and towers (hollow points) versus leading jet $p_{T}$ in data (black) and \pythia p+p simulation (red) dijet events.}
...
The response matrix used in the unfolding procedure is built from \pythia jet-triggered samples applying the kinematic selections outlined in Section~\ref{sec:ueinpp_selection} to the truth and reconstructed level information. 
The reconstructed leading jets are calibrated using the JES correction function derived from a MC-based numerical inversion JES derivation~\cite{ppg09AN} and an additional JER smearing term of 10.1\% is applied to the simulation reconstruction-level jets, determined from differences in the Data/MC JER determined using the dijet bisector and imbalance methods~\cite{ppg08AN}. Finally, truth and calibrated jets are matched with a matching radius requirement of $\Delta R < 0.75R$. 

The transverse region $E_{T}$ is determined by summing topocluster $E_{T}$ in the transverse region: $\pi/3 < |\Delta\phi| < 2\pi/3$ relative to the leading truth jet. 
Truth $\Sigma E_{T}$ uses final-state particles within $|\eta|<1.1$ and $E > 0.5$~GeV for hadrons and $E > 0.2$~GeV for EM particles. These thresholds are chosen to be consistent with other experiments' UE measurements.
...
To reduce prior dependence in the unfolded result, the simulation truth prior distribution is reweighted to match a data-driven pseudo-truth spectrum. 
To get the prior reweighting factors, a single-iteration of the bayesian unfolding is performed for each response matrix variation, nominal and each systematic variation.
This single-unfold pseudo-truth is then normalized and divided by the normalized simulation truth to form a weight matrix. 
These weights are applied to the original truth distribution to produce reweighted prior that serve as the nominal prior distribution for the analysis. 
...
Both a full-closure test, where each response matrix is unfolded against its own measured distribution with one Bayesian iteration, and a half-closure test, where the response from one half-sample is used to unfold the measured distribution from the other half-sample across 1--6 iterations, are performed. 
All response matrices (nominal and systematic variations) are tested for closure. 
...
The full closure and half closure tests for the nominal response matrices are shown in Figures~\ref{fig:full_closure_test_dijet} and \ref{fig:half_closure_test_dijet}. Both the full and half closure tests show agreement between the truth spectra and unfolded spectra of unity within statistical uncertainties. 
...
The unfolding is performed with the response matrices from simulation after both the trimming requirements and prior reweighting procedure for up to 20 iterations to determine the optimal number of iterations in the unfolding process where the change in unfolded result per iteration is at the level of the statistical precision of the measurement. 

Figure~\ref{fig:unfold_iterations} shows this iteration optimization point for unfolding both inclusive jet and dijet events. 
The total change in unfolded values for each iteration is shown in red and the total statistical error on the unfolded result in blue, determined from the quadrature sum of the bin errors in the data histogram and the effect of statistical error in the response matrix tested by sampling 1000 toy models varying the response matrix bin values within 1$\sigma$ of their nominal value.
For both the inclusive jet and dijet results, the change in unfolded values per iteration x total statistical error curve has a minimum around 3 iterations, and thus 3 iterations is chosen as the optimal point for unfolding balancing minimizing prior dependence with minimizing statistical uncertainty in the measurement.
\begin{figure}
    \centering
