...
Three systematic uncertainties for each calorimeter layer \textcolor{red}{To me this reads as if the  *same* three systematic uncertainties were considered for each layer.} were included to account for the uncertainty for the absolute calibration of each of the three calorimeter layers (EMCal, IHCal and OHCal). 
...
to get the MPV nominally \textcolor{red}{the “nominal MPV” ? But in what sense is it nominal?}, the MIP distributions are fit to a gamma function and the MPV is extracted from this fit. 
...
Contributions from each of the calibration variations to the \detdeta measurements are included in detail in Table \ref{table:SystUncertainties} and shown for the \detdeta measurements as a function of $\eta$ in Figures~\ref{fig:eta_dep_syst_emcal}, \ref{fig:eta_dep_syst_hcal} and \ref{fig:eta_dep_syst_calo}. \textcolor{red}{These seem out of order, why do they come after Figure 4.16ff ?}
...
The smallest centrality intervals possible of 1\% were used in this study since the rate of intermittent hot towers is slightly centrality dependent. \textcolor{red}{I did not know this. Do you understand why? Presumably it’s an occupany effect?} 
...
\begin{figure}
    ...
    \caption{\textcolor{red}{The horizontal axis on these plots is labeled as “i $\eta$”, but it’s really just $\eta$, correct?} Variation in reconstruction level $\detdeta$ with and without the inclusion of events with masked intermittently hot towers. Variations for EMCal, HCal, full calorimeter, IHCal-only and OHCal-only reconstructed $\detdeta$ are shown vertically in ascending order. Centrality intervals of 0-1\%, 2-3\%, 57-58\% and 59-60\% are shown in order left to right for each calorimeter measurement of reconstructed $\detdeta$. }
...
For these data to MC comparisons, the simulation physics lists \textcolor{red}{“physics lists” AFAIK is GEANT-specific terminology and should be explained here. For that matter you define Birks law and the relevant constant.} and Birk's constants were varied to determine a range of possible hadronic responses and compared with test beam data collected by the sPHENIX prototypes. 
The full range of possible simulation curves is used in the estimation of the hadronic response uncertainty and this range is estimated to be uniformly distributed across the range of simulation curves \textcolor{red}{This is probably results in an overestiamte of this particular systematic, but this is an observation, not a comment requiring action.} resulting in a final hadronic response uncertainty from these test beam data to MC comparisons of 6.8\%. 
...
Contributions to EM-only energy fraction came from primary $e^{+/-}$, $\gamma$ and $\pi^{0}$ particles, while all other particles contributed to the hadronic energy fraction \textcolor{red}{You may wish to note that “hadronic energy fraction” is not the same as “energy from hadrons”, since muons contribute.}. 
...
\begin{figure}
    ...
    \caption{Fraction of hadronic energy in each calorimeter configuration \textcolor{red}{This description is confusing without more description. Otherwise people will wonder why 0.61+0.97 >> 1.0. I assume that what is meant here for “Had Energy” for a given layer is “the amount of energy IN THIS LAYER that is hadronic in origin”, not “IN THE EVENT”.} found from reweighted \epos events shown for EMCal (top left), HCal (top right), full calorimeter (middle left), IHCal-only (middle right) and OHCal-only (bottom).}
...
Sensitivity to the default \geant hadronic response modeling was tested by using two variations of \geant physics configuration lists to compare to the nominal \geant physics configuration using FTFP\_BERT \textcolor{red}{Needs to be referenced,; these have no meaning for most readers (including me)}. 
Three datasets of \epos events were simulated in the sPHENIX \geant simulation using the physics configurations FTFP\_BERT, FTFP\_BERT\_HP and QGSP\_BERT\_HP \textcolor{red}{Needs to be referenced,; these have no meaning for most readers (including me)}. 
...
\begin{figure}
    ...
    \caption{Reconstructed \detdeta measurements for EMCal (top left), IHCal (top right) \textcolor{red}{Is the “i $\eta$” used as an axis label here explained previously?}, OHCal (bottom left) and full calorimeter (bottom right) in simulation. Comparison between different physics configuration lists, FTFP\_BERT, FTFP\_BERT\_HP and QGSP\_BERT\_HP.}
...
As described in Section \ref{sec:nominal_calorimeter_reconstruction}, within the nominal \textcolor{red}{Is “nominal” needed here?} calorimeter reconstruction pipeline, a zero suppression algorithm is used for processing both the low energy signals and noise. 
...
The EMCal measurement varying the most from changes to the ZS threshold is expected as this switching threshold determines how much noise is processed using the template fit method versus the ZS method in the calorimeter tower reconstruction. \textcolor{red}{The first two sentences in this paragraph are a little awkard. For example, first sentence could start “It is expected that the EmCal measurement woul be most affected . . . “}
There are known biases on the \detdeta measurement from processing noise with template fit method which were studied in depth using Run 2023 data and are included in Section~\ref{sec:zerosuppression} \textcolor{red}{you mean the studies are included in Section 3.3.3, not the biases ;-)} and the EMCal has a much higher overall level of noise compared to the HCal.
Therefore, it is expected that varying the zero suppression to template fit switching threshold would have a larger impact on the EMCal as well as a larger impact in peripherial events where the ratio of signal to noise in the calorimeters should be smaller than in central events. 
...
The averaged total uncertainty on \detdeta from EMCal data is between 5.3\% and 8.1\% and \textcolor{red}{new sentence} the uncertainty on the EMCal \detdeta measurement increases when moving to more peripheral centrality bins. 
...
\begin{figure}
    ...
    \caption{\textcolor{red}{The very clear asymmetry in eta for the most peripheral data is from the ZS? And that is due to what? Beam backgrounds? Support structures?} Systematic uncertainties on \detdeta measurements as a function of $\eta$ for \detdeta measurements from full calorimeter data. Uncertainties from all centralities in measurement are shown in increasing order from top to bottom.}
...
\begin{table}[]
    ...
    \hline
        ...
        60-70\% & 0.4518 \\
    \end{tabular}
    \caption{\textcolor{red}{The values for $\rho$ are probably only valid to two digits, certainly not 4.} Correlation coefficient for EMCal and HCal \detdeta measurements for each centrality measurement in the analysis.}
...
The correlation between the the most forward bins of the measurement ($0.74 < |\eta| < 1.1$) for the full calorimeter measurement for all centrality intervals is shown as a reference in Figures~\ref{fig:calo_eta_covariance}. 
...
\begin{table}
    ...
    \end{tabular}
    \caption{\textcolor{red}{What’s going for 40-50\% centrality? Also, 3 sig figs would be better here.} Correlation coefficient between \detdeta measurements at positive and negative $\eta$ for 3 $|\eta|$ bins of EMCal, HCal and full calorimeter \detdeta measurements for each centrality measurement in the analysis. Here, $\eta_{1}$ refers to $\eta$ bins $0 < |\eta| < 0.36$, $\eta_{2}$ refers to $\eta$ bins $0.36 < |\eta| < 0.74$, and $\eta_{3}$ refers to $\eta$ bins $0.74 < |\eta| < 1.1$.}
...
\begin{figure}
    ...
    \caption{\textcolor{red}{These plots are very hard to interpret because hoizontal and vertical axes are different (in some cases). Please remake with same scales.} Correlation plots for the full calorimeter \detdeta measurements at $0.74 < |\eta| < 1.1$ using each analysis systematic variation for each centrality measurement in the analysis.}
