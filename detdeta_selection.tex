...
This timing resolution translates to a 1-2 cm resolution in the z-vertex position in minimum bias events that is mildly dependent on the event multiplicity, with high multiplicity events having a smaller z-vertex resolution. 
Additionally, the MBD acts as a multiplicity counter, and therefore, the charge information is calibrated in terms of MIP \textcolor{red}{Defined where?} counts.
...
Additiionally, the loose timing cuts applied are well outside the main collision timing distribution.
...
\begin{itemize}
    ...
    \item $\Sigma Q_{MBD,N} > 10 \lor \Sigma Q_{MBD,S} < 150 $ \textcolor{red}{This confuses me. Is it an OR condition?}
...
Figure~\ref{fig:mbd_ns_correlation} shows the region of nonphysical background events \textcolor{red}{Do you understand why this is so asymmetric?} removed using this second requirement on the MBD charge sum. 
...
An energy requirement of 40 GeV is taken since the ZDC response to spectator neutrons is fully efficient at 40 GeV when the ZDC is calibrated so that the single spectator neutron peak is at 100 GeV.
...
Due to modeling limitations in simulation, the ZDC requirement is not included in the selection requirements.
Therefore, this 2\% difference in the minimum bias definition from this ZDC requirement is included in the uncertainty on the minimum bias trigger efficiency. \textcolor{red}{Is the 2\% taken as 1 sigma, 90\% C.L., is it asymmetric?}
...
The second part of the model samples multiplicity distributions from events with set nuclear geometry information, set $\Npart$ and $\Ncoll$, using a negative binomial distribution to get a realistic distribution of particle production and final state multiplicity. \textcolor{red}{Do you have a plot of $\Ncoll$ vs $\Npart$? Is $\Npart$ or $\Ncoll$ used to set NBD mean? The k-parameter is set how?}
The MC Glauber parameters nucleon-nucleon cross-section, nuclear radius, and Woods-Saxon skin thickness are given in Table \ref{tab:glauber} and the MC Glauber $\oplus$ NBD result is fit to the MBD charge sum distribution for the minimum bias trigger's fully efficient region ($\Sigma Q_{MBD} > 150 N_{MIP}$) to determine the best values for the NBD. 
...
The resulting Glauber parameters and extracted $\Npart$ values for eeach centrality interval are included in Table~\ref{tab:glauber}.
...
\begin{figure}
    ...
    \caption{Distributions of $\langle \Npart \rangle$ and $\langle \Ncoll \rangle$ as a function of impact parameter for Cu+Cu and Au+Au collisions at $\sqsntwo$. Mean values are included as lines and event by event fluctuations are included as scatter points. Reproduced from \cite{glauber_review}.}
    \label{fig:npart_ncoll_b}
\end{figure}

\subsection{Centrality Determination in MC}

A minimum bias event selection and centrality percentile determination were also performed on the three MC datasets: \hijing, \ampt and \epos events passed through the \geant simulation.
...
An overall 3\% discrepancy \textcolor{red}{Is the 3\% between the data and all 3 of the generators, or the overall spread between various generators and the data. In other words, are the 3 generators in agreement amongst themselves?} was found between the trigger efficiency extracted from the MC Glauber $\oplus$ NBD fitting to the simulated MBD charge sum distribution and the fraction of generated minimum bias events that pass the minimum bias trigger requirement for events from all three generators.
