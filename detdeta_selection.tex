...
The minimum bias event selection criteria are determined using the global detectors: MBD and ZDC. 
In the MBD, signal is split to read out both the charge and time of each PMT channel where the charge information relates to the amplitude of the signal waveform and the time information relates to the timing of the waveform peak. 
The timing information has $\sim 50$ ps resolution and is used to calculate the z-vertex of the collision. 
This timing resolution translates to a 1-2 cm resolution in the z-vertex position in minimum bias events and is dependent on the event multiplicity, with high multiplicity events having a smaller z-vertex resolution. 
The MBD acts as a multiplicity counter, therefore, the charge information is calibrated in terms of MIP counts.
In addition to the MIP counts calibration, loose requirements on individual MBD PMT charge are imposed to classify a PMT as a "fired" tube in the MBD and for that charge information to be included in the total MBD charge sum used for centrality calibration.
...
The ZDC is included in the minimum bias event selection criteria to help in the removal of beam background events that may still pass the hardware trigger from the MBD. 

\begin{figure}
    \centering
    \includegraphics[width=\linewidth]{figures/chapter4/mbd_hit_criteria_centrality_note.png}
    \caption{MBD PMT charge (left) and time (right) distributions for example individual PMT with PMT hit requirements ($q > 0.5$ and $-20 < t < 20$ ns) included (black lines). Reproduced from \cite{Lis1}.}
...
The first requirement is the hardware level MBD trigger requirement which is also required in the offline reconstruction.
...
Figure~\ref{fig:mbd_ns_correlation} shows the region of nonphysical background events removed using this second requirement on the MBD charge sum. 
The third requirement is a requirement of energy in both sides of the ZDC to remove any remaining beam backgrounds like beam-gas interactions.
An energy requirement of 40 GeV is taken since the ZDC response to spectator neutrons is fully efficient at 40 GeV when the ZDC is calibrated so the single spectator neutron peak is at 100 GeV.
Figure~\ref{fig:zdc_efficiency_run_by_run} shows the run depedence of including this ZDC requirement in the minimum bias criteria.
Here there is a small store dependence, where a store contains a handful of consecutive runs ($\sim$ 10) all within one fill of the RHIC ring with ions. 
There are more significant beam losses at the start of a store than later in the store; 
...
Here there remains a 2\% difference in the minimum bias definition with and without the ZDC requirement which can be due to very peripheral events only having a spectator neutron on one side or the other. 
Due to modeling limitations, in simulation, the hardware trigger requirement is only applied in the offline analysis level and the ZDC requirement is not included in the selection requirements.
The 2\% difference in the minimum bias definition from this ZDC requirement is then included in the uncertainty on the minimum bias trigger efficiency. 
...
Combining all these selections, the total number of events used in the \detdeta measurements of events with centrality between 0 and 70\% correspond to 518,000 events.
...
Events in both data and MC are assigned a centrality value which characterizes the level of geomertric overlap between the two colliding nuclei.
...
This model consists of a MC Glauber simulation which simulates the nuclear geometry event by event and provides nuclear geometry information such as the impact parameter, b, number of participating nucleons, $\Npart$, and number of collisions, $\Ncoll$, 
and a sampling based on a negative binomial distribution from which to sample events with a set $\Npart$ and $\Ncoll$ to get a realistic distribution of particle production and final state multiplicity.
The MC Glauber parameters nucleon-nucleon cross-section, nuclear radius, and Woods-Saxon skin thickness are given in Table \ref{tab:glauber} and the MC Glauber $\oplus$ NBD is fit to the MBD charge sum distribution for the minimum bias trigger's fully efficient region ($\Sigma Q_{MBD} > 150 N_{MIP}$) to determine the best values for the NBD. 
The MBD charge sum distribution with fit to the MC Glauber $\oplus$ NBD model is shown in Figure~\ref{fig:mbd_charge_sum}.
By extracting the integral of the MBD charge sum distribution and fitted MC Glauber $\oplus$ NBD, the minimum bias event selection efficiency was determined to be $93.4\%^{+3.4\%}_{-3.1\%}$ of the Au+Au inelastic cross section and centrality intervals are determined as percentiles of the minimum bias event distribution between 0 and 93\%. 
...
The resulting Glauber parameters for each centrality interval and extracted $\Npart$ values are included in Table~\ref{tab:glauber}.
...
A minimum bias event selection and centrality percentile determination were also performed on the three MC datasets (\texttt{HIJING}, \texttt{AMPT}, and \texttt{EPOS4} events pass through the \textsc{GEANT-4} simulation).
The same MC Glauber $\oplus$ NBD fitting procedure is carried out in simulation, here based on the simulated MBD PMT hits and total charge sum distribution, providing event by event centrality characterization in the same way as in data. 

Additionally, in simulation the efficiency of the minimum bias trigger requirement was also studied. 
An overall 3\% discrepancy was found between the trigger efficiency extracted from the MC Glauber $\oplus$ NBD fitting to the simulated MBD charge sum distribution and the fraction of generated minimum bias events that pass the minimum bias trigger requirement for events from all three generators.
This discrepancy is included in the systematic uncertainty on the minimum bias trigger efficiency. 